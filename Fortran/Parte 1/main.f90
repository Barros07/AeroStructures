...
!---------------------------------------------------------------------------
! ATENÇÃO: A variável FT foi inicialmente declarada como vetor unidimensional:
!     real*8 :: FT(1:2)
! No entanto, na subrotina transpostaF, FT é tratada como matriz bidimensional:
!     real*8, intent(out) :: FT(1,2)
! Isso causou um erro de "Rank mismatch" ao tentar aceder FT(1,:) na main,
! pois estamos a usar sintaxe de matriz (linha completa) numa variável que
! foi declarada como vetor.
!
! SOLUÇÃO: Corrigir a declaração de FT na main para que seja compatível com
! a subrotina transpostaF:
!     real*8 :: FT(1,2)
! Assim, FT passa a ser uma matriz com 1 linha e 2 colunas, permitindo o uso
! correto de FT(1,:) para imprimir a linha completa.
!---------------------------------------------------------------------------
